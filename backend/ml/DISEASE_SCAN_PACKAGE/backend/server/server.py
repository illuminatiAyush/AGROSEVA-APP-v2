...
from .vision.routes import router as vision_router  # Plant Water Stress Scanner (independent)
...
initialize_routes(state, relay_controller, safety_manager, disease_service)
app.include_router(router)
app.include_router(vision_router)   # Plant Water Stress Scanner (independent, no deps)
    ...
    while _decision_loop_running:
        try:
            ...
            if moisture is not None and moisture != _last_processed_moisture:
                ...
                # NOTE: Agent expects moisture in 0-1 scale, Arduino sends 0-100%
                decision = decide_action(
                    moisture=moisture / 100.0,    # Convert % to 0-1 scale
                    irrigation_on=irrigation_on,
                    time_since_last_irr=time_since_last,
                )
                ...
                if config.ENABLE_YIELD:
                    try:
                        ...
                        # Map decision to DRL action format (0, 1, 2, 3)
                        # Use action_int if available (from agent.py), else infer from string
                        drl_action = decision.get("action_int", 0)
                        if drl_action is None or drl_action == 0:
                            if decision["action"] == "IRRIGATE":
                                drl_action = 1  # Default to light irrigation
                            else:
                                drl_action = 0
...
async def startup_event():
    ...
    # Connect to TinyML ESP32 (optional, non-blocking)
    if config.ENABLE_DISEASE:
        print(f"[INIT] Connecting to TinyML ESP32 on {config.TINYML_SERIAL_PORT}...")
        if disease_service.connect():
            print(f"[INIT] TinyML ESP32 connected")
        else:
            print(f"[INIT] TinyML ESP32 not available - disease detection will use backend fallback")
        if disease_service.has_accurate_model():
            print(f"[INIT] Accurate disease model loaded - online mode available")
        else:
            print(f"[INIT] Accurate model not found - place at {config.ACCURATE_MODEL_PATH}")
        if config.GROQ_API_KEY:
            print(f"[INIT] Groq LLM configured - diagnosis available")
        else:
            print(f"[INIT] GROQ_API_KEY not set - LLM diagnosis disabled")
...
def main():
    ...
    print(f"  POST http://localhost:8000/disease")
    print(f"\n🔧 Configuration:")
