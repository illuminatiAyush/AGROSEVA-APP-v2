...
class DiseaseService:
    ...
    def _print_startup_summary(self):
        """Print a clear summary of what is available after startup."""
        print("[DISEASE] ═══════════ Disease Service Startup ═══════════")
        print(f"[DISEASE] Accurate model loaded : {self._accurate_model_loaded}")
        print(f"[DISEASE] Class names loaded    : {len(self._class_names)} classes")
        print(f"[DISEASE] TinyML model loaded   : {self._tinyml_model_loaded}")
        print(f"[DISEASE] ESP32 connected       : {self._connected}")
        if not self._accurate_model_loaded:
            print(f"[DISEASE] ⚠️  No accurate model → disease_name will be None → UI will show 'Unknown'")
            print(f"[DISEASE]    Fix: place model at {config.ACCURATE_MODEL_PATH}")
        if self._accurate_model_loaded and not self._class_names:
            print(f"[DISEASE] ⚠️  Model loaded but class_names_accurate.txt missing → disease_name will be 'class_N'")
        print("[DISEASE] ══════════════════════════════════════════════")
    ...
    def _try_load_accurate_model(self):
        ...
        try:
            ...
            if os.path.exists(class_names_path):
                ...
                print(f"[DISEASE] Loaded {len(self._class_names)} class names")
            else:
                print(f"[DISEASE] WARNING: class_names_accurate.txt not found at {class_names_path}")
                print(f"[DISEASE] Model predictions will use raw class indices instead of names")
    ...
    def run_accurate_inference(self, image_bytes: bytes) -> Dict[str, Any]:
        ...
        try:
            ...
            # Load and convert image to RGB - no special preprocessing
            img = Image.open(io.BytesIO(image_bytes)).convert('RGB')
            ...
            # Resize image — keep raw pixel values [0, 255]
            # Model was trained with raw uint8 values, NOT normalized
            img = img.resize((w, h), Image.LANCZOS)
            img_array = np.array(img, dtype=np.float32)
            img_array = np.expand_dims(img_array, axis=0)
