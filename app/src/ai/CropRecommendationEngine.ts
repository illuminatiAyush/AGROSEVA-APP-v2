...
function generateExplanation(
  ...
  let explanation = `${crop.name} is recommended for the ${season} season. `;
  
  if (excellentMatches >= 5) {
    explanation += 'Most parameters match ideal conditions perfectly. ';
  } else if (excellentMatches >= 3) {
    explanation += 'Several key parameters are in the optimal range. ';
  }
  
  if (goodMatches > 0) {
    explanation += 'Some parameters are slightly off but manageable. ';
  }
  
  if (poorMatches > 0) {
    explanation += 'A few parameters need attention but the crop can still be grown with proper care. ';
  }
  
  explanation += `Overall suitability: ${Math.round(score)}%.`;
...
export function recommendCrop(input: SensorInput): CropRecommendation | null {
  ...
  const recommendations: CropRecommendation[] = suitableCrops.map(crop => {
    ...
    const scores = {
      ...
      temperature: matches.find(m => m.parameter === 'Temperature')?.score || 0,
      humidity: matches.find(m => m.parameter === 'Humidity')?.score || 0,
